...
class highwayNet_g_compose(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.sequential_mlp = nn.Sequential(
            *block(self.encoder_size,self.encoder_size//2),
        )
        self.op_g = torch.nn.Sequential(
            *block(self.encoder_size//2, self.encoder_size//8),
            torch.nn.Linear(self.encoder_size//8, 2),
        )
    ...
    def forward(self, condition, index_div):
        ...
        z_out = self.tanh(self.op_g(sequential_enc))
...
class highwayNet(nn.Module):
    ...
    def __init__(self,args):
        ...
        # Output layers:
        self.op = torch.nn.Linear(self.decoder_size,self.class_num)
