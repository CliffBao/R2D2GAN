...
trSet = ngsimDataset(mat_file=real_set_name, t_f=args_d['in_length'],
...
for start_epoch in range(start_epoch,trainEpochs):
    ...
    for i, data in enumerate(trDataloader):
        ...
        if i%n_critic==0:
            ...
            fm_loss_1 = torch.mean((fake_feature.mean(dim=0)-real_feature.mean(dim=0))**2)
            ...
            fm_loss_2 = torch.mean((fake_seq_feature.mean(dim=0) - real_seq_feature.mean(dim=0))**2)
    ...
    if start_epoch%3==0 and start_epoch<trainEpochs:
        ...
        torch.save(net_g.state_dict(),
                   'trained_models/ngsim_generator_decompose_us1011_nei3_epoch' + str(start_epoch) + '.tar')
...
torch.save(net_g.state_dict(), 'trained_models/ngsim_generator_compose_us1011_nei3.tar')
torch.save(net_d.state_dict(), 'trained_models/ngsim_discriminator_compose_us1011_nei3.tar')
torch.save(net.state_dict(), 'trained_models/ngsim_regression_compose_us1011_nei3.tar')
