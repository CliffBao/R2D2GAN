...
class ngsimDatasetGan(Dataset):
    ...
    def __init__(self, mat_file, t_h=30, t_f=81, d_s=1, neigh_num=-1, enc_size=64, grid_size=(13, 3), hero_grid_pos=19,
                 length=81, use_cuda=True, local_normalise=0, batch_size=512, cond_length=0):
    ...
    def get_all_index(self):
        traj_num = self.neigh_num+1
        all_index = torch.zeros(len(self.D),2*traj_num)
        for i in range(len(self.D)):
            nbrs, maneuver, scale = self.__getitem__(i)
            count = 0
            for id, nbr in enumerate(nbrs):
                if len(nbr) != 0:
                    x = id % self.grid_size[0]
                    y = id // self.grid_size[0]
                    all_index[i,2*count+0] = (y-1)/1
                    all_index[i,2*count+1] = (x-6.0)/6.0
                    count += 1
            assert count == traj_num
        return all_index
    ...
    def __getitem__(self, idx):
        ...
        maneuver = maneuver + np.array([1, 0])
        x_maneuver = np.zeros((1, 3))
        y_maneuver = np.zeros((1, 2))
        x_maneuver[0, maneuver[0].astype(int)] = 1
        y_maneuver[0, maneuver[1].astype(int)] = 1
        maneuver_onehot = np.concatenate((x_maneuver, y_maneuver), axis=1)

        hero_maneuver = maneuver_onehot
        ref_index = 0
        ...
        for id, i in enumerate(grid):
            ...
            maneuver_all.append(maneuver_onehot)
            if nei.shape[0] > 0:
        ...
        return neigh_real, maneuver_all, hero_max
    ...
    def getHistoryEM(self, vehId, t, dsId):
        ...
        stpt = np.argwhere(vehTrack[:, 0] == t).item()
        enpt = np.minimum(len(vehTrack), np.argwhere(vehTrack[:, 0] == t).item() + self.t_f)
        if stpt >= enpt:
    ...
    def collate_fn(self, samples):
        ...
        nbr_batch_size = 0
        for nbrs, _, _ in samples:
            nbr_batch_size += sum([len(nbrs[i]) != 0 for i in range(len(nbrs))])
            # nbr_batch_size += (real.shape[1]-2)
        maxlen = self.t_f // self.d_s
        ...
        maneuver_batch = torch.zeros(nbr_batch_size, 5)
        scale_batch = []
        ...
        for sampleId, (nbrs, maneuver, scale) in enumerate(samples):
        ...
        return nbrs_real_batch, mask_batch, index_division, maneuver_batch, scale_batch


class composedDataset(Dataset):
    def __init__(self, mat_file,traj_num=4,length=81):
        self.D = scp.loadmat(mat_file)['data']
        self.all_index = scp.loadmat(mat_file)['label']
    ...
    def __getitem__(self, idx):
        return self.D[idx,:],self.all_index[idx,:]

    def collate_fn(self, samples):
        data_batch = torch.zeros(len(samples), self.traj_num*2*self.length)
        index_division = [None] * len(samples)
        pos_condition = torch.zeros(len(samples), self.traj_num*2)
        for sampleId, (data,label) in enumerate(samples):
            # Set up neighbor, neighbor sequence length, and mask batches:
            index = []
            data_batch[sampleId,:] = torch.from_numpy(data)
            pos_condition[sampleId,:] = torch.from_numpy(label)
            for i in range(self.traj_num):
                index.append(sampleId*self.traj_num+i)
            index_division[sampleId] = index

        return data_batch, index_division,pos_condition
